...
# Este proyecto no versiona ninguna credencial: pasala con -ServiceAccountJson o exporta
# GOOGLE_APPLICATION_CREDENTIALS. El .gitignore de PDFReader solo cubre *-service-account*.json,
# asi que un JSON con otro nombre dentro del repo acabaria en un commit.
$DefaultServiceAccountJson = if (-not [string]::IsNullOrWhiteSpace($env:GOOGLE_APPLICATION_CREDENTIALS)) {
    $env:GOOGLE_APPLICATION_CREDENTIALS
}
else {
    Join-Path $ProjectRoot 'google-play-service-account.json'
}
...
$DefaultTargetFramework = 'net10.0-android36.0'
$AccessToken = $null
...
Write-Host '========================================'
Write-Host ' PDF Reader - Publish AAB to Play Console'
Write-Host '========================================'
Write-Host
...
$defaultReleaseName = if ($defaultDisplayVersion -and $defaultVersionCode) {
    "PDF Reader $defaultDisplayVersion ($defaultVersionCode)"
}
elseif ($defaultVersionCode) {
    "PDF Reader ($defaultVersionCode)"
}
else {
    "PDF Reader $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
}
