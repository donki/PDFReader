    ...
    private void OnPanUpdated(object? sender, PanUpdatedEventArgs e)
    {
        switch (e.StatusType)
        {
            ...
            case GestureStatus.Running:
                ...
                _panLastTotalY = e.TotalY;

                // Con zoom, arrastrar mueve la pagina. Sin zoom no se mueve (se reserva para pasar pagina).
                if (_zoom > MinZoom + 0.01)
                {
            ...
            case GestureStatus.Completed:
            case GestureStatus.Canceled:
                // Sin zoom, un arrastre horizontal claro cambia de pagina. Se hace aqui, sobre el
                // propio Pan, porque en Android un SwipeGestureRecognizer que convive con el Pan no
                // llega a dispararse (el Pan consume los eventos tactiles).
                if (_zoom <= MinZoom + 0.01)
                {
                    const double threshold = 55; // dips minimos de arrastre horizontal
                    if (Math.Abs(_panLastTotalX) > threshold &&
                        Math.Abs(_panLastTotalX) > Math.Abs(_panLastTotalY) * 1.3)
                    {
                        if (_panLastTotalX < 0)
                            OnNextClicked(sender, e);     // arrastre a la izquierda -> pagina siguiente
                        else
                            OnPreviousClicked(sender, e); // arrastre a la derecha -> pagina anterior
                    }
                }
                _panLastTotalX = 0;
                _panLastTotalY = 0;
